...
std::ostream& binary_write_code(std::ostream& o, const code& c, char start, char sep)
{
	bool needSep = false;

	o << start;
	if (c.gtin.defined())		{ if (needSep) { o << sep; } binary_write(c.gtin, o); needSep = false; } // If separator needed, tell it.
	if (c.fablot.defined())		{ if (needSep) { o << sep; } binary_write(c.fablot, o); needSep = true; } // If separator needed, tell it.
	if (c.itemComponent.defined())	{ if (needSep) { o << sep; } binary_write(c.itemComponent, o); needSep = false; } // If separator needed, tell it.
...
public:
        binary_serializable(code& c, char startChar, char sepChar) : c_(c), startChar_(startChar), sepChar_(sepChar) { }
...
public:
        binary_serializable_const(const code& c, char startChar, char sepChar) : c_(c), startChar_(startChar), sepChar_(sepChar) { }
...
template<typename CodeT>
typename std::enable_if<std::is_const<CodeT>::value, binary_serializable_const>::type
binary(CodeT c, char start, char sep)
{
...
template<typename CodeT>
typename std::enable_if<!std::is_const<CodeT>::value, binary_serializable>::type
binary(CodeT c, char start, char sep)
{
